...
*   **Detalles del código y uso del Workspace**:
    ...
    *   **Selectores de Espacio y Estilo**: Permiten al usuario alternar entre estilos de diseño arquitectónico bajo la opción de "Apartamento Completo (Todo Junto)", la cual genera planos texturizados en 3D a color.
    ...
    *   **Base de datos de renders**: Contiene una colección indexada de renders en 3D de alta fidelidad correspondientes al apartamento completo a escala (`/test-images/rendered_apartment.png`), permitiendo alinearse perfectamente con el plano original.
...
*   **Qué hace y dónde está**: Es un objeto clave-valor definido al inicio del archivo (fuera del componente). Asocia cada estilo de diseño arquitectónico con el render del plano de planta 3D del apartamento completo (`/test-images/rendered_apartment.png`) para alinearse de forma precisa con el plano técnico de origen.
*   **Por qué y para qué**: Permite realizar pruebas visuales inmediatas y gratuitas. En lugar de consumir recursos externos en la fase de prueba, asocia renders coherentes de tipo plano 3D que encajan con la vista previa del plano del usuario, garantizando un mantenimiento y consumo de coste cero.
