...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-800/80 bg-slate-950/75 backdrop-blur-md transition-all duration-300">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative flex h-11 w-11 items-center justify-center rounded-xl bg-slate-900 border border-indigo-500/30 transition-transform group-hover:scale-105">
              ...
              <span className="absolute -inset-1 -z-10 rounded-xl bg-indigo-500/20 opacity-0 blur-md transition-opacity group-hover:opacity-100" />
            </div>
            {/* Texto de la marca con gradiente */}
            <span className="bg-gradient-to-r from-white via-slate-100 to-indigo-200 bg-clip-text text-xl font-extrabold tracking-wide text-transparent">
              Roomifi
            </span>
          </Link>
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    {isActive && (
                      <span className="absolute bottom-0 left-0 h-[2px] w-full rounded-full bg-indigo-400 shadow-[0_0_8px_rgba(99,102,241,0.8)]" />
                    )}
                  </>
                )}
              </NavLink>
            ...
            {isCheckingAuth ? (
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="flex items-center gap-2 rounded-full bg-slate-900/80 p-1.5 pr-4 border border-slate-800 hover:border-slate-700 transition-colors"
                >
                  {/* Inicial del usuario con fondo degradado */}
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-tr from-indigo-500 to-pink-500 text-sm font-bold text-white uppercase">
                ...
                {isProfileDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-52 origin-top-right rounded-xl border border-slate-800 bg-slate-950 p-1.5 shadow-2xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="px-3 py-2 text-xs text-slate-500 border-b border-slate-900 mb-1">
                      Conectado como {user.username}
                    </div>
                    <button
                      onClick={handleSignOut}
                      className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-pink-400 hover:bg-slate-900 transition-colors"
                    >
      ...
      {isMobileMenuOpen && (
        <div className="border-t border-slate-900 bg-slate-950 px-4 py-6 md:hidden animate-in slide-in-from-top duration-300">
          <nav className="flex flex-col gap-4">
            ...
            <div className="my-2 h-px bg-slate-900" />
            ...
            {isCheckingAuth ? (
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3 py-2">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-tr from-indigo-500 to-pink-500 text-sm font-bold text-white">
                    {user.username.charAt(0)}
                  </div>
                  <span className="text-sm font-medium text-slate-300">{user.username}</span>
                </div>
                <Button variant="secondary" className="w-full justify-start text-pink-400" onClick={handleSignOut}>
                  Cerrar Sesión
                </Button>
              </div>
            ) : (
